...
namespace DaOAuthV2.Service.Test.Fake
{
    public class FakeCodeRepository : ICodeRepository
    {
        ...
        {
            var c = FakeDataBase.Instance.Codes.Where(co => co.CodeValue.Equals(code)).FirstOrDefault();
            if (c == null)
            {
                return null;
            }

            var uc = FakeDataBase.Instance.UsersClient.Where(uscl => uscl.Id.Equals(c.UserClientId)).FirstOrDefault();
            if (uc == null)
            {
                return c;
            }
